...
public class URLClassLoader extends SecureClassLoader implements Closeable {
	 ...
	 * manager's {@code checkCreateClassLoader} method to ensure creation of a class
	 * loader is allowed.
	 *
	 * @param urls   the URLs from which to load classes and resources
	 * @param parent the parent class loader for delegation
	 * @exception SecurityException    if a security manager exists and its
	 *                                 {@code checkCreateClassLoader} method doesn't
	 ...
	 * delegation parent {@code ClassLoader}. The URLs will be searched in the order
	 * specified for classes and resources after first searching in the parent class
	 * loader. Any URL that ends with a '/' is assumed to refer to a directory.
	 * Otherwise, the URL is assumed to refer to a JAR file which will be downloaded
	...
	/*
	 * A map (used as a set) to keep track of closeable local resources (either
	 * JarFiles or FileInputStreams). We don't care about Http resources since they
	 * don't need to be closed.
	 *
	 * If the resource is coming from a jar file we keep a (weak) reference to the
	 * JarFile object which can be closed if URLClassLoader.close() called. Due to
	 * jar file caching there will typically be only one JarFile object per
	 * underlying jar file.
	 *
	 * For file resources, which is probably a less common situation we have to keep
	 * a weak reference to each stream.
	 */
	 ...
	 * by it. If another thread is loading a class when the {@code close} method is
	 * invoked, then the result of that load is undefined.
	 * <p>
	 * The method makes a best effort attempt to close all opened files, by catching
	 * {@link IOException}s internally. Unchecked exceptions and errors are not
	 * caught. Calling close on an already closed loader has no effect.
	 * <p>
	 * 
	 * @exception IOException       if closing any file opened by this class loader
	 *                              resulted in an IOException. Any such exceptions
	 *                              are caught internally. If only one is caught,
	 *                              then it is re-thrown. If more than one exception
	 *                              is caught, then the second and following
	 *                              exceptions are added as suppressed exceptions of
	 ...
	 * resources.
	 * <p>
	 * If the URL specified is {@code null} or is already in the list of URLs, or if
	 * this loader is closed, then invoking this method has no effect.
	 *
	 ...
	 * @exception ClassNotFoundException if the class could not be found, or if the
	 *                                   loader is closed.
	...
	/*
	 * Retrieve the package using the specified package name. If non-null, verify
	 * the package using the specified code source and manifest.
	 */
	 ...
	 * The resulting Class must be resolved before it can be used.
	 */
	...
	/**
	 * Defines a new package by name in this ClassLoader. The attributes contained
	 * in the specified Manifest will be used to obtain package version and sealing
	 * information. For sealed packages, the additional URL specifies the code
	 * source URL from which the package was loaded.
	 *
	 ...
	 *                                     existing package either in this class
	 *                                     loader or one of its ancestors
...
//		Attributes attr = SharedSecrets.javaUtilJarAccess().getTrustedAttributes(man,
//				name.replace('.', '/').concat("/"));
	...
	/*
	 * Returns true if the specified package name is sealed according to the given
	 * manifest.
	 *
...
//				name.replace('.', '/').concat("/"));
	 ...
	 * @return a {@code URL} for the resource, or {@code null} if the resource could
	 *         not be found, or if the loader is closed.
	 */
	 ...
	 * @return an {@code Enumeration} of {@code URL}s If the loader is closed, the
	 ...
	 * of this method first calls super.getPermissions and then adds permissions
	 ...
	 * permission to connect to and accept connections from that authority may be
	 * granted. If the protocol is "file" and the path specifies a file, then
	 * permission to read that file is granted. If protocol is "file" and the path
	 * is a directory, permission is granted to read all files and (recursively) all
	...
	protected PermissionCollection getPermissions(CodeSource codesource) {
		...
		if (p instanceof FilePermission) {
			...
			/**
			 * Not loading from a 'file:' URL so we want to give the class permission to
			 * connect to and accept from the remote host after we've made sure the host is
			 * the correct one and is valid.
			 */
			URL locUrl = url;
	 ...
	 * class loader. If a security manager is installed, the {@code loadClass}
	 * method of the URLClassLoader returned by this method will invoke the
	 * {@code SecurityManager.checkPackageAccess} method before loading the class.
	 *
	 ...
	 * {@code loadClass} method of the URLClassLoader returned by this method will
	 * invoke the {@code SecurityManager.checkPackageAccess} before loading the
	 * class.
	 *
	 * @param urls the URLs to search for classes and resources
	 * @exception NullPointerException if {@code urls} is {@code null}.
