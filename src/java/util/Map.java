...
public interface Map<K, V> {
	 ...
	 * @throws UnsupportedOperationException if the <tt>putAll</tt> operation is not
	 *                                       supported by this map
	...
	interface Entry<K, V> {
		 ...
		 *                                       the entry has been removed from the
		 *                                       backing map.
		 */
		V setValue(V value);
		 ...
		 * <p>
		 * The returned comparator is serializable and throws
		 * {@link NullPointerException} when comparing an entry with null values.
		 *
	 ...
	 * been processed or the action throws an exception. Unless otherwise specified
	 * by the implementing class, actions are performed in the order of entry set
	 ...
	 *                                         null, and this map does not permit
	 *                                         null keys or values (<a href=
	 *                                         "{@docRoot}/java/util/Collection.html#optional-restrictions">optional</a>)
	 * @throws IllegalArgumentException        if some property of a replacement
	 ...
	 *           </pre>
	 *
	 *           The default implementation does not throw NullPointerException for
	 *           maps that do not support null values if oldValue is null unless
	 ...
	 * mapping is recorded. The most common usage is to construct a new object
	 ...
	 *
	 * <p>
	 * Or to implement a multi-value map, {@code Map<K,Collection<V>>}, supporting
	 ...
	 *
	 *
	 * @implSpec The default implementation is equivalent to the following steps for
	 *           this {@code map}, then returning the current value or {@code null}
	 *           if now absent:
	 *
	 *           <pre>
	 *  {@code
	 ...
	 *           synchronization or atomicity properties of this method. Any
	 *           implementation providing atomicity guarantees must override this
	 *           method and document its concurrency properties. In particular, all
	 *           implementations of subinterface
	 *           {@link java.util.concurrent.ConcurrentMap} must document whether
	 *           the function is applied once atomically only if the value is not
	 *           present.
	 *
	 * @param key             key with which the specified value is to be associated
	 ...
	 * itself throws an (unchecked) exception, the exception is rethrown, and the
	 * current mapping is left unchanged.
	 *
	 * @implSpec The default implementation is equivalent to performing the
	 *           following steps for this {@code map}, then returning the current
	 *           value or {@code null} if now absent:
	 *
	 *           <pre>
	 *  {@code
	 * if (map.get(key) != null) {
	 ...
	 * value (or {@code null} if there is no current mapping). For example, to
	 * either create or append a {@code String} msg to a value mapping:
	 *
	 * <pre>
	 *  {@code
	 ...
	 * removes if the result is {@code null}. This method may be of use when
	 * combining multiple mapped values for a key. For example, to either create or
