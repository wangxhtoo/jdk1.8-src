...
public abstract class AbstractSet<E> extends AbstractCollection<E> implements Set<E> {
	 ...
	 * <tt>true</tt> if the given object is also a set, the two sets have the same
	 * size, and every member of the given set is contained in this set. This
	 * ensures that the <tt>equals</tt> method works properly across different
	 * implementations of the <tt>Set</tt> interface.
	 * <p>
	 *
	 * This implementation first checks if the specified object is this set; if so
	 * it returns <tt>true</tt>. Then, it checks if the specified object is a set
	 * whose size is identical to the size of this set; if not, it returns false. If
	 * so, it returns <tt>containsAll((Collection) o)</tt>.
	 *
	 ...
	 * <tt>s1.equals(s2)</tt> implies that <tt>s1.hashCode()==s2.hashCode()</tt> for
	 * any two sets <tt>s1</tt> and <tt>s2</tt>, as required by the general contract
	 ...
	 * specified collection, by invoking the <tt>size</tt> method on each. If this
	 ...
	 * contained in the specified collection. If it is so contained, it is removed
	 * from this set with the iterator's <tt>remove</tt> method. If the specified
	 * collection has fewer elements, then the implementation iterates over the
	 * specified collection, removing from this set each element returned by the
	 * iterator, using this set's <tt>remove</tt> method.
	 *
	 * <p>
