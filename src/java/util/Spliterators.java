...
public final class Spliterators {
	...
	/**
	 * Creates a {@code Spliterator} covering the elements of a given array, using a
	 * customized set of spliterator characteristics.
	 *
	 * <p>
	 * This method is provided as an implementation convenience for Spliterators
	 * which store portions of their elements in arrays, and need fine control over
	 * Spliterator characteristics. Most other situations in which a Spliterator for
	 * an array is needed should use {@link Arrays#spliterator(Object[])}.
	 *
	 * <p>
	 * The returned spliterator always reports the characteristics {@code SIZED} and
	 * {@code SUBSIZED}. The caller may provide additional characteristics for the
	 * spliterator to report; it is common to additionally specify {@code IMMUTABLE}
	 * and {@code ORDERED}.
	 *
	 * @param                           <T> Type of elements
	 * @param array                     The array, assumed to be unmodified during
	 *                                  use
	...
	/**
	 * Creates a {@code Spliterator} covering a range of elements of a given array,
	 ...
	 *                                        {@code toIndex} is less than
	 *                                        {@code fromIndex}, or {@code toIndex}
	 ...
	 * which store portions of their elements in arrays, and need fine control over
	 * Spliterator characteristics. Most other situations in which a Spliterator for
	 * an array is needed should use {@link Arrays#spliterator(int[])}.
	 *
	 * <p>
	 * The returned spliterator always reports the characteristics {@code SIZED} and
	 * {@code SUBSIZED}. The caller may provide additional characteristics for the
	 ...
	 * which store portions of their elements in arrays, and need fine control over
	 * Spliterator characteristics. Most other situations in which a Spliterator for
	 * an array is needed should use {@link Arrays#spliterator(int[], int, int)}.
	 *
	 * <p>
	 * The returned spliterator always reports the characteristics {@code SIZED} and
	 * {@code SUBSIZED}. The caller may provide additional characteristics for the
	 ...
	 * which store portions of their elements in arrays, and need fine control over
	 * Spliterator characteristics. Most other situations in which a Spliterator for
	 * an array is needed should use {@link Arrays#spliterator(long[])}.
	 *
	 * <p>
	 * The returned spliterator always reports the characteristics {@code SIZED} and
	 * {@code SUBSIZED}. The caller may provide additional characteristics for the
	 ...
	 * which store portions of their elements in arrays, and need fine control over
	 * Spliterator characteristics. Most other situations in which a Spliterator for
	 * an array is needed should use {@link Arrays#spliterator(long[], int, int)}.
	 *
	 * <p>
	 * The returned spliterator always reports the characteristics {@code SIZED} and
	 * {@code SUBSIZED}. The caller may provide additional characteristics for the
	 * spliterator to report. (For example, if it is known the array will not be
	 * further modified, specify {@code IMMUTABLE}; if the array data is considered
	 * to have an an encounter order, specify {@code ORDERED}). The method
	 * {@link Arrays#spliterator(long[], int, int)} can often be used instead, which
	 * returns a spliterator that reports {@code SIZED}, {@code SUBSIZED},
	 * {@code IMMUTABLE}, and {@code ORDERED}.
	 *
	 * @param array                     The array, assumed to be unmodified during
	 *                                  use
	 * @param fromIndex                 The least index (inclusive) to cover
	 ...
	 * which store portions of their elements in arrays, and need fine control over
	 * Spliterator characteristics. Most other situations in which a Spliterator for
	 * an array is needed should use {@link Arrays#spliterator(double[])}.
	 *
	 * <p>
	 * The returned spliterator always reports the characteristics {@code SIZED} and
	 * {@code SUBSIZED}. The caller may provide additional characteristics for the
	 ...
	 * which store portions of their elements in arrays, and need fine control over
	 * Spliterator characteristics. Most other situations in which a Spliterator for
	 * an array is needed should use {@link Arrays#spliterator(double[], int, int)}.
	 *
	 * <p>
	 * The returned spliterator always reports the characteristics {@code SIZED} and
	 * {@code SUBSIZED}. The caller may provide additional characteristics for the
	...
	/**
	 * Validate inclusive start index and exclusive end index against the length of
	 * an array.
	 * 
	 * @param arrayLength The length of the array
	 * @param origin      The inclusive start index
	 * @param fence       The exclusive end index
	 * @throws ArrayIndexOutOfBoundsException if the start index is greater than the
	 *                                        end index, if the start index is
	 *                                        negative, or the end index is greater
	 ...
	 * <p>
	 * The spliterator is
	 * <em><a href="Spliterator.html#binding">late-binding</a></em>, inherits the
	...
	/**
	 * Creates a {@code Spliterator} using a given {@code Iterator} as the source of
	 * elements, and with a given initially reported size.
	 *
	 * <p>
	 * The spliterator is not
	 * <em><a href="Spliterator.html#binding">late-binding</a></em>, inherits the
	 * <em>fail-fast</em> properties of the iterator, and implements
	 * {@code trySplit} to permit limited parallelism.
	 *
	 * <p>
	 ...
	 * @param iterator        The iterator for the source
	 * @param characteristics Characteristics of this spliterator's source or
	 *                        elements ({@code SIZED} and {@code SUBSIZED}, if
	 *                        supplied, are ignored and are not reported.)
	 * @return A spliterator from an iterator
	 * @throws NullPointerException if the given iterator is {@code null}
	 */
	...
	/**
	 * Creates a {@code Spliterator.OfInt} using a given
	 * {@code IntStream.IntIterator} as the source of elements, with no initial size
	 * estimate.
	 *
	 * <p>
	 * The spliterator is not
	...
	/**
	 * Creates a {@code Spliterator.OfLong} using a given
	 * {@code LongStream.LongIterator} as the source of elements, with no initial
	 * size estimate.
	 *
	 * <p>
	 * The spliterator is not
	 ...
	 * maintained in an unmodifiable {@code Object[]} array.
	 */
	static final class ArraySpliterator<T> implements Spliterator<T> {
		/**
		 * The array, explicitly typed as Object[]. Unlike in some other classes (see
		 * for example CR 6260652), we do not need to screen arguments to ensure they
		 * are exactly of type Object[] so long as no methods write into the array or
		 * serialize it, which we ensure here by defining this class as final.
		 */
	...
	/**
	 * An abstract {@code Spliterator} that implements {@code trySplit} to permit
	 * limited parallelism.
	 *
	 * <p>
	 * An extending class need only implement
	 * {@link #tryAdvance(java.util.function.Consumer) tryAdvance}. The extending
	 * class should override {@link #forEachRemaining(java.util.function.Consumer)
	 ...
	 *          parallelism, is to create a spliterator from an iterator (see
	 *          {@link #spliterator(Iterator, long, int)}. Depending on the
	 *          circumstances using an iterator may be easier or more convenient
	 *          than extending this class, such as when there is already an iterator
	 *          available to use.
	 *
	...
	public static abstract class AbstractSpliterator<T> implements Spliterator<T> {
		 ...
		 *
		 * @param est                       the estimated size of this spliterator if
		 *                                  known, otherwise {@code Long.MAX_VALUE}.
		 * @param additionalCharacteristics properties of this spliterator's source or
		 *                                  elements. If {@code SIZED} is reported then
		 *                                  this spliterator will additionally report
		 *                                  {@code SUBSIZED}.
		 */
	 ...
	 * {@link #tryAdvance(java.util.function.IntConsumer)} tryAdvance}. The
	 * extending class should override
	 * {@link #forEachRemaining(java.util.function.IntConsumer)} forEach} if it can
	 ...
	 *          An alternative to using this class, that also permits limited
	 *          parallelism, is to create a spliterator from an iterator (see
	 *          {@link #spliterator(java.util.PrimitiveIterator.OfInt, long, int)}.
	 *          Depending on the circumstances using an iterator may be easier or
	 *          more convenient than extending this class. For example, if there is
	 *          already an iterator available to use then there is no need to extend
	 *          this class.
	 *
	...
	/**
	 * An abstract {@code Spliterator.OfLong} that implements {@code trySplit} to
	 * permit limited parallelism.
	 *
	 * <p>
	 * To implement a spliterator an extending class need only implement
	 * {@link #tryAdvance(java.util.function.LongConsumer)} tryAdvance}. The
	 * extending class should override
	 * {@link #forEachRemaining(java.util.function.LongConsumer)} forEach} if it can
	 * provide a more performant implementation.
	 *
	 * @apiNote This class is a useful aid for creating a spliterator when it is not
	 *          possible or difficult to efficiently partition elements in a manner
	 ...
	 *          An alternative to using this class, that also permits limited
	 *          parallelism, is to create a spliterator from an iterator (see
	 *          {@link #spliterator(java.util.PrimitiveIterator.OfLong, long, int)}.
	 *          Depending on the circumstances using an iterator may be easier or
	 *          more convenient than extending this class. For example, if there is
	 *          already an iterator available to use then there is no need to extend
	 *          this class.
	...
	/**
	 * An abstract {@code Spliterator.OfDouble} that implements {@code trySplit} to
	 * permit limited parallelism.
	 *
	 * <p>
	 * To implement a spliterator an extending class need only implement
	 * {@link #tryAdvance(java.util.function.DoubleConsumer)} tryAdvance}. The
	 * extending class should override
	 * {@link #forEachRemaining(java.util.function.DoubleConsumer)} forEach} if it
	 ...
	 *          An alternative to using this class, that also permits limited
	 *          parallelism, is to create a spliterator from an iterator (see
	 *          {@link #spliterator(java.util.PrimitiveIterator.OfDouble, long, int)}.
	 *          Depending on the circumstances using an iterator may be easier or
	 *          more convenient than extending this class. For example, if there is
	 *          already an iterator available to use then there is no need to extend
	 *          this class.
	...
	/**
	 * A Spliterator using a given Iterator for element operations. The spliterator
	...
	static class IteratorSpliterator<T> implements Spliterator<T> {
		...
		/**
		 * Creates a spliterator using the given given collection's
		...
		/**
		 * Creates a spliterator using the given iterator for traversal, and reporting
		 * the given initial size and characteristics.
		 *
		 * @param iterator        the iterator for the source
	 ...
	 * operations. The spliterator implements {@code trySplit} to permit limited
	 * parallelism.
	 */
