...
public class StringTokenizer implements Enumeration<Object> {
	 ...
	 * highest value. It is used to optimize the detection of delimiter characters.
	 *
	 * It is unlikely to provide any optimization benefit in the hasSurrogates case
	 * because most string characters will be smaller than the limit, but we keep it
	 * so that the two code paths remain similar.
	 */
	private int maxDelimCodePoint;

	/**
	 * If delimiters include any surrogates (including surrogate pairs),
	 * hasSurrogates is true and the tokenizer uses the different code path. This is
	 * because String.indexOf(int) doesn't handle unpaired surrogates as a single
	 * character.
	 */
	 ...
	 * delimiter characters are also returned as tokens. Each delimiter is returned
	 * as a string of length one. If the flag is <code>false</code>, the delimiter
	 * characters are skipped and only serve as separators between tokens.
	 * <p>
	 * Note that if <tt>delim</tt> is <tt>null</tt>, this constructor does not throw
	 * an exception. However, trying to invoke other methods on the resulting
	 * <tt>StringTokenizer</tt> may result in a <tt>NullPointerException</tt>.
	 *
	...
	/**
	 * Skips delimiters starting from the specified position. If retDelims is false,
	...
	public boolean hasMoreTokens() {
		/*
		 * Temporarily store this position and use it in the following nextToken()
		 * method only if the delimiters haven't been changed in that nextToken()
		 * invocation.
		 */
	...
	public String nextToken() {
		/*
		 * If next position already computed in hasMoreElements() and delimiters have
		 * changed between the computation and this invocation, then use the computed
		 * value.
		 */
	 ...
	 * object is changed to be the characters in the string <tt>delim</tt>. Then the
	 * next token in the string after the current position is returned. The current
	 * position is advanced beyond the recognized token. The new delimiter set
	...
	/**
	 * Returns the same value as the <code>hasMoreTokens</code> method. It exists so
	...
	/**
	 * Returns the same value as the <code>nextToken</code> method, except that its
	 * declared return value is <code>Object</code> rather than <code>String</code>.
	 * It exists so that this class can implement the <code>Enumeration</code>
	 * interface.
	 *
