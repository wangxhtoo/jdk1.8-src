 ...
 *
 * <p>
 * Unless otherwise stated, streams are created as sequential streams. A
...
final class Streams {
	...
	/**
	 * An object instance representing no value, that cannot be an actual data
	 * element of a stream. Used when processing streams that can contain
	...
	static final class RangeIntSpliterator implements Spliterator.OfInt {
		...
		/**
		 * The spliterator size below which the spliterator will be split at the
		 * mid-point to produce balanced splits. Above this size the spliterator will be
		 * split at a ratio of 1:(RIGHT_BALANCED_SPLIT_RATIO - 1) to produce
		 * right-balanced splits.
		 *
		 * <p>
		 * Such splitting ensures that for very large ranges that the left side of the
		 * range will more likely be processed at a lower-depth than a balanced tree at
		 * the expense of a higher-depth for the right side of the range.
		 *
		 * <p>
		 * This is optimized for cases such as IntStream.ints() that is implemented as
		 * range of 0 to Integer.MAX_VALUE but is likely to be augmented with a limit
		 * operation that limits the number of elements to a count lower than this
		 * threshold.
		 */
		private static final int BALANCED_SPLIT_THRESHOLD = 1 << 24;

		/**
		 * The split ratio of the left and right split when the spliterator size is
		 * above BALANCED_SPLIT_THRESHOLD.
		 */
	 ...
	 *
	 * This implementation cannot be used for ranges whose size is greater than
	 * Long.MAX_VALUE
	 */
	static final class RangeLongSpliterator implements Spliterator.OfLong {
		 ...
		 *
		 * <p>
		 * This is optimized for cases such as LongStream.longs() that is implemented as
		 * range of 0 to Long.MAX_VALUE but is likely to be augmented with a limit
		 * operation that limits the number of elements to a count lower than this
		 * threshold.
		 */
	...
	abstract static class ConcatSpliterator<T, T_SPLITR extends Spliterator<T>> implements Spliterator<T> {
		...
		public int characteristics() {
			if (beforeSplit) {
				...
				return aSpliterator.characteristics() & bSpliterator.characteristics() & ~(Spliterator.DISTINCT
						| Spliterator.SORTED | (unsized ? Spliterator.SIZED | Spliterator.SUBSIZED : 0));
			} else {
	...
	/**
	 * Given two Runnables, return a Runnable that executes both in sequence, even
	 * if the first throws an exception, and if both throw exceptions, add any
	 * exceptions thrown by the second as suppressed exceptions of the first.
	 */
	 ...
	 * {@link BaseStream#close} methods in sequence, even if the first throws an
	 * exception, and if both throw exceptions, add any exceptions thrown by the
	 * second as suppressed exceptions of the first.
	 */
