 ...
 * <div>
 * <table>
 * <caption>Type Characteristics</caption> <thead class="tableSubHeadingColor">
 * <tr>
 * <th colspan="2">&nbsp;</th>
 * <th>{@code DISTINCT}</th>
 * <th>{@code SORTED}</th>
 ...
 *
 * <p>
 * In the above table, "PCI" means "may preserve, clear, or inject"; "PC" means
 * "may preserve or clear", "PI" means "may preserve or inject", and "N" means
 * "not valid".
 *
 * <p>
 * Stream flags are represented by unioned bit sets, so that a single word may
 * describe all the characteristics of a given stream entity, and that, for
 * example, the flags for a stream source can be efficiently combined with the
 * flags for later operations on that stream.
 *
 * <p>
 * The bit masks {@link #STREAM_MASK}, {@link #OP_MASK}, and
 ...
 * that stream has; when describing a stream operation, one need describe
 * whether the operation preserves, injects, or clears that characteristic.
 * Accordingly, two bits are used for each flag, so as to allow representing not
 * only the presence of of a characteristic, but how an operation modifies that
 * characteristic. There are two common forms in which flag bits are combined
 * into an {@code int} bit set. <em>Stream flags</em> are a unioned bit set
 * constructed by ORing the enum characteristic values of {@link #set()} (or,
 * more commonly, ORing the corresponding static named constants prefixed with
 * {@code IS_}). <em>Operation flags</em> are a unioned bit set constructed by
 * ORing the enum characteristic values of {@link #set()} or {@link #clear()}
 * (to inject, or clear, respectively, the corresponding flag), or more commonly
 * ORing the corresponding named constants prefixed with {@code IS_} or
 * {@code NOT_}. Flags that are not marked with {@code IS_} or {@code NOT_} are
 * implicitly treated as preserved. Care must be taken when combining bitsets
 * that the correct combining operations are applied in the correct order.
 *
 * <p>
 ...
 * subset of stream flags of that stream. For example:
 * 
 * <pre>
 *  {@code
 *     Spliterator s = ...;
 ...
 * operation characteristics. Preservation means that if that characteristic is
 * present on the input, then it is also present on the output. Clearing means
 * that the characteristic is not present on the output regardless of the input.
 * Injection means that the characteristic is present on the output regardless
 * of the input. If a characteristic is not cleared or injected then it is
...
enum StreamOpFlag {
	...
	/*
	 * Each characteristic takes up 2 bits in a bit set to accommodate preserving,
	 * clearing and setting/injecting information.
	 *
	 * This applies to stream flags, intermediate/terminal operation flags, and
	 * combined stream and operation flags. Even though the former only requires 1
	 * bit of information per characteristic, is it more efficient when combining
	 * flags to align set and inject bits.
	 *
	 * Characteristics belong to certain types, see the Type enum. Bit masks for the
	 * types are constructed as per the following table:
	 *
	 * DISTINCT SORTED ORDERED SIZED SHORT_CIRCUIT SPLITERATOR 01 01 01 01 00 STREAM
	 * 01 01 01 01 00 OP 11 11 11 10 01 TERMINAL_OP 00 00 10 00 01
	 * UPSTREAM_TERMINAL_OP 00 00 10 00 00
	 *
	 * 01 = set/inject 10 = clear 11 = preserve
	 *
	 * Construction of the columns is performed using a simple builder for non-zero
	 * values.
	 */
	 ...
	 * in a stream {@code x, y}, {@code !x.equals(y)}.
	 * <p>
	 * A stream may have this value or an intermediate operation can preserve, clear
	 * or inject this value.
	 */
	// 0, 0x00000001
	...
	/**
	 * Characteristic value signifying that encounter order follows a natural sort
	 * order of comparable elements.
	 * <p>
	 * A stream can have this value or an intermediate operation can preserve, clear
	 * or inject this value.
	 * <p>
	 * Note: The {@link java.util.Spliterator#SORTED} characteristic can define a
	 * sort order with an associated non-null comparator. Augmenting flag state with
	 * addition properties such that those properties can be passed to operations
	 * requires some disruptive changes for a singular use-case. Furthermore,
	 * comparing comparators for equality beyond that of identity is likely to be
	 * unreliable. Therefore the {@code SORTED} characteristic for a defined
	 * non-natural sort order is not mapped internally to the {@code SORTED} flag.
	 */
	// 1, 0x00000004
	...
	/**
	 * Characteristic value signifying that an encounter order is defined for stream
	 * elements.
	 * <p>
	 * A stream can have this value, an intermediate operation can preserve, clear
	 * or inject this value, or a terminal operation can preserve or clear this
	 * value.
	 */
	// 2, 0x00000010
	...
	/**
	 * Characteristic value signifying that size of the stream is of a known finite
	 * size that is equal to the known finite size of the source spliterator input
	 * to the first stream in the pipeline.
	 * <p>
	 ...
	 * stream.
	 * <p>
	 * An intermediate operation can preserve or inject this value, or a terminal
	 * operation can preserve or inject this value.
	 */
	// 12, 0x01000000
	...
	enum Type {
		...
		/**
		 * The flag is associated with terminal operation flags that are propagated
		 * upstream across the last stateful operation boundary
		 */
	...
	/**
	 * The mask table for a flag, this is used to determine if a flag corresponds to
	 * a certain flag type and for creating mask constants.
	 */
	...
	/**
	 * Checks if this flag is set on stream flags, injected on operation flags, and
	 * injected on combined stream and operation flags.
	 *
	...
	/**
	 * The initial value to be combined with the stream flags of the first stream in
	 * the pipeline.
	 */
	 ...
	 * <p>
	 * A flag set on stream flags or injected on operation flags, and injected
	 * combined stream and operation flags, will be injected on the updated combined
	 * stream and operation flags.
	 *
	 * <p>
	 * A flag set on stream flags or injected on operation flags, and cleared on the
	 * combined stream and operation flags, will be cleared on the updated combined
	 * stream and operation flags.
	 *
	 * <p>
	 * A flag set on the stream flags or injected on operation flags, and preserved
	 * on the combined stream and operation flags, will be injected on the updated
	 * combined stream and operation flags.
	 *
	 * <p>
	 * A flag not set on the stream flags or cleared/preserved on operation flags,
	 * and injected on the combined stream and operation flags, will be injected on
	 * the updated combined stream and operation flags.
	 *
	 * <p>
	 * A flag not set on the stream flags or cleared/preserved on operation flags,
	 * and cleared on the combined stream and operation flags, will be cleared on
	 * the updated combined stream and operation flags.
	 *
	 * <p>
	 * A flag not set on the stream flags, and preserved on the combined stream and
	 * operation flags will be preserved on the updated combined stream and
	 * operation flags.
	 *
	 * <p>
	 * A flag cleared on operation flags, and preserved on the combined stream and
	 * operation flags will be cleared on the updated combined stream and operation
	 * flags.
	 *
	 * <p>
	 * A flag preserved on operation flags, and preserved on the combined stream and
	 * operation flags will be preserved on the updated combined stream and
	 * operation flags.
	 *
	 * @param newStreamOrOpFlags the stream or operation flags.
	 * @param prevCombOpFlags    previously combined stream and operation flags. The
	 *                           value {#link INITIAL_OPS_VALUE} must be used as the
	 *                           seed value.
	 ...
	 *
	 * <p>
	 * Each flag injected on the combined stream and operation flags will be set on
	 * the stream flags.
	 *
	...
	static int toStreamFlags(int combOpFlags) {
		...
		// Shift left 1 to restore set flags and mask off anything other than the set
		// flags
	 ...
	 *           {@code Comparator} is {@code null}) then the characteristic is
	 *           converted to the {@link #SORTED} flag, otherwise the characteristic
	 *           is not converted.
	 *
	 * @param spliterator the spliterator from which to obtain characteristic bit
	 *                    set.
	 * @return the stream flags.
	 */
