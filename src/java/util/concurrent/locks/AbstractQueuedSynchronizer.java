 ...
 * {@link UnsupportedOperationException}. Implementations of these methods must
 * be internally thread-safe, and should in general be short and not block.
...
public abstract class AbstractQueuedSynchronizer extends AbstractOwnableSynchronizer implements java.io.Serializable {
	...
	static final class Node {
		...
		/**
		 * Status field, taking on only the values: SIGNAL: The successor of this node
		 * is (or will soon be) blocked (via park), so the current node must unpark its
		 * successor when it releases or cancels. To avoid races, acquire methods must
		 * first indicate they need a signal, then retry the atomic acquire, and then,
		 * on failure, block. CANCELLED: This node is cancelled due to timeout or
		 * interrupt. Nodes never leave this state. In particular, a thread with
		 * cancelled node never again blocks. CONDITION: This node is currently on a
		 * condition queue. It will not be used as a sync queue node until transferred,
		 * at which time the status will be set to 0. (Use of this value here has
