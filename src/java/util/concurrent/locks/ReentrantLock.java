...
public class ReentrantLock implements Lock, java.io.Serializable {
	...
	abstract static class Sync extends AbstractQueuedSynchronizer {
		...
		/**
		 * Performs {@link Lock#lock}. The main reason for subclassing is to allow fast
		 * path for nonfair version.
		 */
		abstract void lock();

		/**
		 * Performs non-fair tryLock. tryAcquire is implemented in subclasses, but both
		 * need nonfair try for trylock method.
		 */
	...
	/**
	 * Acquires the lock unless the current thread is {@linkplain Thread#interrupt
	 * interrupted}.
	 *
	 * <p>
	 * Acquires the lock if it is not held by another thread and returns
	 ...
	 * with the value {@code true}, setting the lock hold count to one. Even when
	 ...
	 * available, whether or not other threads are currently waiting for the lock.
	 ...
	 * this lock, then use {@link #tryLock(long, TimeUnit) tryLock(0,
	 ...
	 * <p>
	 * Acquires the lock if it is not held by another thread and returns immediately
	 * with the value {@code true}, setting the lock hold count to one. If this lock
	 * has been set to use a fair ordering policy then an available lock <em>will
	 * not</em> be acquired if any other threads are waiting for the lock. This is
	 * in contrast to the {@link #tryLock()} method. If you want a timed
	 * {@code tryLock} that does permit barging on a fair lock then combine the
	 ...
	 *
	 * <p>
	 * If the lock is acquired then the value {@code true} is returned and the lock
	 ...
	 * signalling} methods are called, then an {@link IllegalMonitorStateException}
	 * is thrown.
	 *
	 * <li>When the condition {@linkplain Condition#await() waiting} methods are
	 * called the lock is released and, before they return, the lock is reacquired
	 ...
	 * purposes. For example, if a certain section of code should not be entered
	 * with the lock already held then we can assert that fact:
	 *
	 * <pre>
	 * {
	 * 	&#64;code
	 ...
	 *
	 * <p>
	 * Analogous to the {@link Thread#holdsLock(Object)} method for built-in monitor
	 * locks, this method is typically used for debugging and testing. For example,
	 * a method that should only be called while a lock is held can assert that this
	 * is the case:
	 *
	 * <pre>
	 * {
	...
	/**
	 * Queries if this lock is held by any thread. This method is designed for use
	 ...
	 * guarantee that any other thread will ever acquire this lock. This method is
	 ...
	 * because cancellations may occur at any time, a {@code true} return does not
	 * guarantee that this thread will ever acquire this lock. This method is
	 * designed primarily for use in monitoring of the system state.
	 *
	 ...
	 * dynamically while this method traverses internal data structures. This method
	 * is designed for use in monitoring of the system state, not for
	 ...
	 * time, a {@code true} return does not guarantee that a future {@code signal}
	 ...
	 * associated with this lock. Note that because timeouts and interrupts may
	 ...
	 * given condition associated with this lock. Because the actual set of threads
	 * may change dynamically while constructing this result, the returned
