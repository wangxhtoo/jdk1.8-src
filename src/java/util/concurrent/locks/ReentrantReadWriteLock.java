...
public class ReentrantReadWriteLock implements ReadWriteLock, java.io.Serializable {
	...
	/**
	 * Creates a new {@code ReentrantReadWriteLock} with default (nonfair) ordering
	 * properties.
	 */
	...
	abstract static class Sync extends AbstractQueuedSynchronizer {
		 ...
		 * logically divided into two unsigned shorts: The lower one representing the
		 * exclusive (writer) lock hold count, and the upper the shared (reader) hold
		 * count.
		 */
		...
		/**
		 * The number of reentrant read locks held by current thread. Initialized only
		 * in constructor and readObject. Removed whenever a thread's read hold count
		 * drops to 0.
		 * 
		 * 本地线程计数器
		 */
		 ...
		 *
		 * <p>
		 * Can outlive the Thread for which it is caching the read hold count, but
		 * avoids garbage retention by not retaining a reference to the Thread.
		 *
		 * <p>
		 * Accessed via a benign data race; relies on the memory model's final field and
		 ...
		 *
		 * <p>
		 * More precisely, firstReader is the unique thread that last changed the shared
		 * count from 0 to 1, and has not released the read lock since then; null if
		 * there is no such thread.
		 *
		 * <p>
		 * Cannot cause garbage retention unless the thread terminated without
		 * relinquishing its read locks, since tryReleaseShared sets it to null.
		 *
		 * <p>
		 * Accessed via a benign data race; relies on the memory model's out-of-thin-air
		 * guarantees for references.
		 *
		 * <p>
		 * This allows tracking of read holds for uncontended read locks to be very
		 * cheap.
		 */
		// 第一个读线程
		...
		/*
		 * Acquires and releases use the same code for fair and nonfair locks, but
		 * differ in whether/how they allow barging when queues are non-empty.
		 */

		/**
		 * Returns true if the current thread, when trying to acquire the read lock, and
		 * otherwise eligible to do so, should block because of policy for overtaking
		...
		protected final boolean tryRelease(int releases) {
			...
			if (free)
				// 若写锁的新线程数为0，则将锁的持有则设置为null
				setExclusiveOwnerThread(null);
			// 设置写锁的新线程数，不管独占模式是否被释放，更新独占重入数
			setState(nextc);
			return free;
		}
		...
		protected final boolean tryAcquire(int acquires) {
			/*
			 * Walkthrough: 1. If read count nonzero or write count nonzero and owner is a
			 * different thread, fail. 2. If count would saturate, fail. (This can only
			 * happen if count is already nonzero.) 3. Otherwise, this thread is eligible
			 * for lock if it is either a reentrant acquire or queue policy allows it. If
			 * so, update state and set owner.
			 */
		...
		protected final int tryAcquireShared(int unused) {
			/*
			 * Walkthrough: 1. If write lock held by another thread, fail. 2. Otherwise,
			 ...
			 * non-reentrant case. 3. If step 2 fails either because thread apparently not
			 * eligible or CAS fails or count saturated, chain to version with full retry
			 * loop.
			 */
			Thread current = Thread.currentThread();
			int c = getState();
			// 如果写锁线程数！=0，且独占锁不是当前线程则返回失败，因为存在锁🔒降级
			if (exclusiveCount(c) != 0 && getExclusiveOwnerThread() != current)
				return -1;
			// 读锁数量
			int r = sharedCount(c);
			// 读锁是否需要等待（公平锁原则） 持有线程小于最大数（65535）
		...
		final int fullTryAcquireShared(Thread current) {
			/*
			 * This code is in part redundant with that in tryAcquireShared but is simpler
			 * overall by not complicating tryAcquireShared with interactions between
		...
		/**
		 * Performs tryLock for read, enabling barging in both modes. This is identical
		 * in effect to tryAcquireShared except for lack of calls to readerShouldBlock.
		 */
	...
	static final class NonfairSync extends Sync {
		...
		final boolean readerShouldBlock() {
			/*
			 * As a heuristic to avoid indefinite writer starvation, block if the thread
			 * that momentarily appears to be head of queue, if one exists, is a waiting
			 * writer. This is only a probabilistic effect since a new reader will not block
			 * if there is a waiting writer behind other enabled readers that have not yet
	...
	public static class ReadLock implements Lock, java.io.Serializable {
		 ...
		 *
		 * <li>is {@linkplain Thread#interrupt interrupted} while acquiring the read
		 * lock,
		 *
		 * </ul>
		 *
		...
		/**
		 * Acquires the read lock only if the write lock is not held by another thread
		 * at the time of invocation.
		 *
		 * <p>
		 * Acquires the read lock if the write lock is not held by another thread and
		 * returns immediately with the value {@code true}. Even when this lock has been
		 * set to use a fair ordering policy, a call to {@code tryLock()} <em>will</em>
		 * immediately acquire the read lock if it is available, whether or not other
		 * threads are currently waiting for the read lock. This &quot;barging&quot;
		 ...
		 * {@link #tryLock(long, TimeUnit) tryLock(0, TimeUnit.SECONDS) } which is
		 ...
		 * <p>
		 * Acquires the read lock if the write lock is not held by another thread and
		 * returns immediately with the value {@code true}. If this lock has been set to
		 * use a fair ordering policy then an available lock <em>will not</em> be
		 * acquired if any other threads are waiting for the lock. This is in contrast
		 * to the {@link #tryLock()} method. If you want a timed {@code tryLock} that
		 * does permit barging on a fair lock then combine the timed and un-timed forms
		 * together:
		 *
		 * <pre>
		 *  {@code
		...
		/**
		 * Throws {@code UnsupportedOperationException} because {@code ReadLocks} do not
		 * support conditions.
		 *
		 ...
		 * in brackets, includes the String {@code "Read locks ="} followed by the
	...
	public static class WriteLock implements Lock, java.io.Serializable {
		 ...
		 *
		 * <p>
		 * If the write lock is acquired by the current thread then the lock hold count
		 * is set to one.
		 *
		 * <p>
		 * If the current thread:
		 ...
		 * <li>has its interrupted status set on entry to this method; or
		 *
		 * <li>is {@linkplain Thread#interrupt interrupted} while acquiring the write
		 * lock,
		 *
		 * </ul>
		 *
		...
		/**
		 * Acquires the write lock only if it is not held by another thread at the time
		 * of invocation.
		 *
		 * <p>
		 * Acquires the write lock if neither the read nor write lock are held by
		 * another thread and returns immediately with the value {@code true}, setting
		 * the write lock hold count to one. Even when this lock has been set to use a
		 * fair ordering policy, a call to {@code tryLock()} <em>will</em> immediately
		 * acquire the lock if it is available, whether or not other threads are
		 * currently waiting for the write lock. This &quot;barging&quot; behavior can
		 * be useful in certain circumstances, even though it breaks fairness. If you
		 ...
		 * waiting time and the current thread has not been {@linkplain Thread#interrupt
		 * interrupted}.
		 *
		 * <p>
		 * Acquires the write lock if neither the read nor write lock are held by
		 * another thread and returns immediately with the value {@code true}, setting
		 * the write lock hold count to one. If this lock has been set to use a fair
		 * ordering policy then an available lock <em>will not</em> be acquired if any
		 * other threads are waiting for the write lock. This is in contrast to the
		 * {@link #tryLock()} method. If you want a timed {@code tryLock} that does
		 * permit barging on a fair lock then combine the timed and un-timed forms
		 * together:
		 *
		 * <pre>
		 *  {@code
		 ...
		 *
		 * <p>
		 * If the write lock is acquired then the value {@code true} is returned and the
		 ...
		 * <p>
		 * The returned {@link Condition} instance supports the same usages as do the
		 * {@link Object} monitor methods ({@link Object#wait() wait},
		 * {@link Object#notify notify}, and {@link Object#notifyAll notifyAll}) when
		 * used with the built-in monitor lock.
		 *
		 * <ul>
		 *
		 * <li>If this write lock is not held when any {@link Condition} method is
		 * called then an {@link IllegalMonitorStateException} is thrown. (Read locks
		 * are held independently of write locks, so are not checked or affected.
		 ...
		 * threads that could unblock it will not be able to acquire the write lock.)
		 *
		 ...
		 * reacquired and the lock hold count restored to what it was when the method
		 * was called.
		 *
		 * <li>If a thread is {@linkplain Thread#interrupt interrupted} while waiting
		 * then the wait will terminate, an {@link InterruptedException} will be thrown,
		 ...
		 * the default case not specified, but for <em>fair</em> locks favors those
		 ...
		 * to {@link ReentrantReadWriteLock#isWriteLockedByCurrentThread}.
		 *
		 * @return {@code true} if the current thread holds this lock and {@code false}
		 *         otherwise
		 * @since 1.6
		 */
	...
	/**
	 * Returns the thread that currently owns the write lock, or {@code null} if not
	 * owned. When this method is called by a thread that is not the owner, the
	 * return value reflects a best-effort approximation of current lock status. For
	 * example, the owner may be momentarily {@code null} even if there are threads
	 * trying to acquire the lock but have not yet done so. This method is designed
	 * to facilitate construction of subclasses that provide more extensive lock
	 ...
	 * Note that because cancellations may occur at any time, a {@code true} return
	 * does not guarantee that any other thread will ever acquire a lock. This
	 ...
	 * with the write lock. Note that because timeouts and interrupts may occur at
	 ...
	 * collection are in no particular order. This method is designed to facilitate
	 * construction of subclasses that provide more extensive condition monitoring
	 * facilities.
	 *
	 * @param condition the condition
	 * @return the collection of threads
	...
	/**
	 * Returns a string identifying this lock, as well as its lock state. The state,
	 * in brackets, includes the String {@code "Write locks ="} followed by the
	 * number of reentrantly held write locks, and the String {@code "Read locks ="}
	 ...
	 * rather than via method Thread.getId() because getId() is not final, and has
	 * been known to be overridden in ways that do not preserve unique mappings.
	 */
