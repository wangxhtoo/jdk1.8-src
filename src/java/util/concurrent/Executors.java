...
public class Executors {
	...
	/**
	 * Returns an object that delegates all defined {@link ExecutorService} methods
	 * to the given executor, but not any other methods that might otherwise be
	 * accessible using casts. This provides a way to safely "freeze" configuration
	 * and disallow tuning of a given concrete implementation.
	 * 
	...
	/**
	 * Returns an object that delegates all defined {@link ScheduledExecutorService}
	 * methods to the given executor, but not any other methods that might otherwise
	 * be accessible using casts. This provides a way to safely "freeze"
	 ...
	 * creates all new threads used by an Executor in the same {@link ThreadGroup}.
	 * If there is a {@link java.lang.SecurityManager}, it uses the group of
	 * {@link System#getSecurityManager}, else the group of the thread invoking this
	 * {@code defaultThreadFactory} method. Each new thread is created as a
	 * non-daemon thread with priority set to the smaller of
	 * {@code Thread.NORM_PRIORITY} and the maximum priority permitted in the thread
	 * group. New threads have names accessible via {@link Thread#getName} of
	 * <em>pool-N-thread-M</em>, where <em>N</em> is the sequence number of this
	 * factory, and <em>M</em> is the sequence number of the thread created by this
	 * factory.
	 * 
	 ...
	 * settings as {@link Executors#defaultThreadFactory}, additionally setting the
	 * AccessControlContext and contextClassLoader of new threads to be the same as
	 * the thread invoking this {@code privilegedThreadFactory} method. A new
	 * {@code privilegedThreadFactory} can be created within an
	 * {@link AccessController#doPrivileged AccessController.doPrivileged} action
	 * setting the current thread's access control context to create threads with
	 * the selected permission settings holding within that action.
	 *
	 * <p>
	 * Note that while tasks running within such threads will have the same access
	 * control and class loader settings as the current thread, they need not have
	 * the same {@link java.lang.ThreadLocal} or
	 * {@link java.lang.InheritableThreadLocal} values. If necessary, particular
	 * values of thread locals can be set or reset before any task runs in
	 * {@link ThreadPoolExecutor} subclasses using
	 * {@link ThreadPoolExecutor#beforeExecute(Thread, Runnable)}. Also, if it is
	 ...
	 *
	 * @return a thread factory
	 * @throws AccessControlException if the current access control context does not
	 *                                have permission to both get and set context
	 *                                class loader
	 */
	...
	/**
	 * Returns a {@link Callable} object that, when called, runs the given task and
	 * returns the given result. This can be useful when applying methods requiring
	 * a {@code Callable} to an otherwise resultless action.
	 * 
	...
	/**
	 * Returns a {@link Callable} object that will, when called, execute the given
	 * {@code callable} under the current access control context. This method should
	 * normally be invoked within an {@link AccessController#doPrivileged
	 * AccessController.doPrivileged} action to create callables that will, if
	 * possible, execute under the selected permission settings holding within that
	...
	/**
	 * Returns a {@link Callable} object that will, when called, execute the given
	 * {@code callable} under the current access control context, with the current
	 * context class loader as the context class loader. This method should normally
	 * be invoked within an {@link AccessController#doPrivileged
	 * AccessController.doPrivileged} action to create callables that will, if
	 * possible, execute under the selected permission settings holding within that
	 * action; or if not possible, throw an associated
	 * {@link AccessControlException}.
	 ...
	 * @throws NullPointerException   if callable null
	 * @throws AccessControlException if the current access control context does not
	 *                                have permission to both set and get context
	 *                                class loader
	 */
	...
	/**
	 * A wrapper class that exposes only the ScheduledExecutorService methods of a
