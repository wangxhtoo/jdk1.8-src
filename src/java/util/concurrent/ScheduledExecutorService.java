 ...
 *
 * <p>
 * The {@code schedule} methods create tasks with various delays and return a task object that can be used to cancel or check execution. The
 * {@code scheduleAtFixedRate} and {@code scheduleWithFixedDelay} methods create and execute tasks that run periodically until cancelled.
 *
 * <p>
 * Commands submitted using the {@link Executor#execute(Runnable)} and {@link ExecutorService} {@code submit} methods are scheduled with a requested
 * delay of zero. Zero and negative delays (but not periods) are also allowed in {@code schedule} methods, and are treated as requests for immediate
 * execution.
 *
 * <p>
 * All {@code schedule} methods accept <em>relative</em> delays and periods as arguments, not absolute times or dates. It is a simple matter to
 * transform an absolute time represented as a {@link java.util.Date} to the required form. For example, to schedule at a certain future {@code date},
 * you can use: {@code schedule(task,
 * date.getTime() - System.currentTimeMillis(),
 * TimeUnit.MILLISECONDS)}. Beware however that expiration of a relative delay need not coincide with the current {@code Date} at which the task is
...
public interface ScheduledExecutorService extends ExecutorService {
	 ...
	 *
	 * @param command      the task to execute
	 * @param initialDelay the time to delay first execution 初始化第一个任务的延迟时间
	 * @param delay        the delay between the termination of one execution and the commencement of the next 两个任务间隔的延迟时间
