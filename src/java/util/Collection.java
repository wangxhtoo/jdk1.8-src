 ...
 * unchecked exception, typically <tt>NullPointerException</tt> or
 * <tt>ClassCastException</tt>. Attempting to query the presence of an
 ...
 * <tt>e</tt> such that <tt>(o==null ? e==null : o.equals(e))</tt>." This
 * specification should <i>not</i> be construed to imply that invoking
 * <tt>Collection.contains</tt> with a non-null argument <tt>o</tt> will cause
 * <tt>o.equals(e)</tt> to be invoked for any element <tt>e</tt>.
 * Implementations are free to implement optimizations whereby the
 * <tt>equals</tt> invocation is avoided, for example, by first comparing the
 * hash codes of the two elements. (The {@link Object#hashCode()} specification
 ...
 *
 * @implSpec The default method implementations (inherited or otherwise) do not
...
public interface Collection<E> extends Iterable<E> {
	 ...
	 * operation). Returns <tt>true</tt> if this collection changed as a result of
	 * the call. (Returns <tt>false</tt> if this collection does not permit
	 ...
	 * restrictions on the type of elements that may be added. Collection classes
	 * should clearly specify in their documentation any restrictions on what
	 ...
	 * (rather than returning <tt>false</tt>). This preserves the invariant that a
	 ...
	 * collection contains one or more such elements. Returns <tt>true</tt> if this
	 ...
	 * @return <tt>true</tt> if this collection changed as a result of the call
	 * @throws UnsupportedOperationException if the <tt>removeAll</tt> method is not
	 *                                       supported by this collection
	 ...
	 *
	 * @implSpec The default implementation traverses all elements of the collection
	 *           using its {@link #iterator}. Each matching element is removed using
	 *           {@link Iterator#remove()}. If the collection's iterator does not
	 *           support removal then an {@code UnsupportedOperationException} will
	 ...
	 * <tt>Collection</tt> interface "directly" (in other words, create a class that
	 * is a <tt>Collection</tt> but is not a <tt>Set</tt> or a <tt>List</tt>) must
	 * exercise care if they choose to override the <tt>Object.equals</tt>. It is
	 * not necessary to do so, and the simplest course of action is to rely on
	 * <tt>Object</tt>'s implementation, but the implementor may wish to implement a
	 * "value comparison" in place of the default "reference comparison." (The
	 * <tt>List</tt> and <tt>Set</tt> interfaces mandate such value comparisons.)
	 * <p>
	 *
	 * The general contract for the <tt>Object.equals</tt> method states that equals
	 * must be symmetric (in other words, <tt>a.equals(b)</tt> if and only if
	 * <tt>b.equals(a)</tt>). The contracts for <tt>List.equals</tt> and
	 * <tt>Set.equals</tt> state that lists are only equal to other lists, and sets
	 * to other sets. Thus, a custom <tt>equals</tt> method for a collection class
	 * that implements neither the <tt>List</tt> nor <tt>Set</tt> interface must
	 * return <tt>false</tt> when this collection is compared to any list or set.
	 * (By the same logic, it is not possible to write a class that correctly
	 * implements both the <tt>Set</tt> and <tt>List</tt> interfaces.)
	 *
	 ...
	 * <tt>Collection</tt> interface adds no stipulations to the general contract
	 * for the <tt>Object.hashCode</tt> method, programmers should take note that
	 * any class that overrides the <tt>Object.equals</tt> method must also override
	 * the <tt>Object.hashCode</tt> method in order to satisfy the general contract
	 * for the <tt>Object.hashCode</tt> method. In particular,
	 ...
	 * for the {@link #stream()} and {@link #parallelStream()}} methods,
	 * spliterators should either have the characteristic of {@code IMMUTABLE} or
	 ...
	 *           additional characteristic values, beyond that of {@code SIZED} and
	 *           {@code SUBSIZED}, does not aid clients to control, specialize or
	 *           simplify computation. However, this does enable shared use of an
	 *           immutable and empty spliterator instance (see
	 *           {@link Spliterators#emptySpliterator()}) for empty collections, and
	 *           enables clients to determine if such a spliterator covers no
	 *           elements.
	 *
