 ...
 * provides whatever data bytes are written to the piped output stream. Typically, data is read from
 * a <code>PipedInputStream</code> object by one thread and data is written to the corresponding
 * <code>PipedOutputStream</code> by some other thread. Attempting to use both objects from a single
 * thread is not recommended, as it may deadlock the thread. The piped input stream contains a
 * buffer, decoupling read operations from write operations, within limits. A pipe is said to be
 * <a name="BROKEN"> <i>broken</i> </a> if a thread that was providing data bytes to the connected
...
public class PipedInputStream extends InputStream {
	...
	/*
	 * REMIND: identification of the read and write sides needs to be more sophisticated. Either using
	 * thread groups (but what about pipes within a thread?) or using finalization (but it may be a long
	 * time until the next GC).
	 */
	Thread readSide;
	Thread writeSide;
	 ...
	 * when received from the connected piped output stream. <code>in&lt;0</code> implies the buffer is
	 ...
	 * <code>src</code>. Data bytes written to <code>src</code> will then be available as input from
	 * this stream.
	 *
	...
	/**
	 * Creates a <code>PipedInputStream</code> so that it is connected to the piped output stream
	 * <code>src</code> and uses the specified pipe size for the pipe's buffer. Data bytes written to
	...
	/**
	 * Creates a <code>PipedInputStream</code> so that it is not yet
	 * {@linkplain #connect(java.io.PipedOutputStream) connected} and uses the specified pipe size for
	 * the pipe's buffer. It must be
	 * {@linkplain java.io.PipedOutputStream#connect( java.io.PipedInputStream) connected} to a
	 * <code>PipedOutputStream</code> before being used.
	 *
	 * @param pipeSize the size of the pipe's buffer.
	 ...
	 * is thrown.
	 * <p>
	 * If <code>src</code> is an unconnected piped output stream and <code>snk</code> is an unconnected
	 ...
	 *                        {@link #connect(java.io.PipedOutputStream) unconnected}, closed, or if an
	 *                        I/O error occurs.
	 * @since JDK1.1
	 */
	 ...
	 * <code>int</code> in the range <code>0</code> to <code>255</code>. This method blocks until input
	 ...
	 *                        <a href="#BROKEN"> <code>broken</code></a>, closed, or if an I/O error
	 *                        occurs.
	 */
	 ...
	 * <code>len</code> exceeds the pipe's buffer size. If <code>len </code> is zero, then no bytes are
	 * read and 0 is returned; otherwise, the method blocks until at least 1 byte of input is available,
	 ...
	 *         or if the pipe is {@link #connect(java.io.PipedOutputStream) unconnected}, or
