 ...
 * <code>Thread</code> object, the new thread has its priority initially set
 * equal to the priority of the creating thread, and is a daemon thread if and
 * only if the creating thread is a daemon.
 * <p>
 * When a Java Virtual Machine starts up, there is usually a single non-daemon
 * thread (which typically calls the method named <code>main</code> of some
 * designated class). The Java Virtual Machine continues to execute threads
 * until either of the following occurs:
 * <ul>
 * <li>The <code>exit</code> method of class <code>Runtime</code> has been
 * called and the security manager has permitted the exit operation to take
 * place.
 * <li>All threads that are not daemon threads have died, either by returning
 * from the call to the <code>run</code> method or by throwing an exception that
 * propagates beyond the <code>run</code> method.
 * </ul>
 * <p>
...
public class Thread implements Runnable {
	...
	/*
	 * The requested stack size for this thread, or 0 if the creator did not specify
	 * a stack size. It is up to the VM to do whatever it likes with this number;
	 * some VMs will ignore it.
	 */
	...
	/*
	 * The object in which this thread is blocked in an interruptible I/O operation,
	 * if any. The blocker's interrupt method should be invoked after setting this
	 ...
	 *
	 * @throws IllegalArgumentException if the value of {@code millis} is negative,
	 *                                  or the value of {@code nanos} is not in the
	 *                                  range {@code 0-999999}
	 *
	 * @throws InterruptedException     if any thread has interrupted the current
	 *                                  thread. The <i>interrupted status</i> of the
	 ...
	 * @param stackSize           the desired stack size for the new thread, or zero
	 *                            to indicate that this parameter is to be ignored.
	...
	private void init(ThreadGroup g, Runnable target, String name, long stackSize, AccessControlContext acc,
		...
		if (g == null) {
			...
			/*
			 * If the security doesn't have a strong opinion of the matter use the parent
			 * thread group.
			 */
			if (g == null) {
	...
	/**
	 * Allocates a new {@code Thread} object. This constructor has the same effect
	 * as {@linkplain #Thread(ThreadGroup,Runnable,String) Thread}
	 * {@code (null, null, gname)}, where {@code gname} is a newly generated name.
	 * Automatically generated names are of the form {@code "Thread-"+}<i>n</i>,
	 ...
	 * Allocates a new {@code Thread} object. This constructor has the same effect
	 * as {@linkplain #Thread(ThreadGroup,Runnable,String) Thread}
	 * {@code (null, target, gname)}, where {@code gname} is a newly generated name.
	 * Automatically generated names are of the form {@code "Thread-"+}<i>n</i>,
	 * where <i>n</i> is an integer.
	 *
	...
	/**
	 * Creates a new Thread that inherits the given AccessControlContext. This is
	...
	/**
	 * Allocates a new {@code Thread} object so that it has {@code target} as its
	 * run object, has the specified {@code name} as its name, and belongs to the
	 * thread group referred to by {@code group}.
	 *
	 * <p>
	 * If there is a security manager, its
	 * {@link SecurityManager#checkAccess(ThreadGroup) checkAccess} method is
	 * invoked with the ThreadGroup as its argument.
	 *
	 * <p>
	 * In addition, its {@code checkPermission} method is invoked with the
	 * {@code RuntimePermission("enableContextClassLoaderOverride")} permission when
	 * invoked directly or indirectly by the constructor of a subclass which
	 * overrides the {@code getContextClassLoader} or {@code setContextClassLoader}
	 * methods.
	 *
	 * <p>
	 ...
	 * {@linkplain #setPriority setPriority} may be used to change the priority to a
	 * new value.
	 *
	 * <p>
	 ...
	 * method {@linkplain #setDaemon setDaemon} may be used to change whether or not
	 * a thread is a daemon.
	 *
	 * @param group  the thread group. If {@code null} and there is a security
	 *               manager, the group is determined by
	 *               {@linkplain SecurityManager#getThreadGroup
	 ...
	 * Allocates a new {@code Thread} object so that it has {@code target} as its
	 * run object, has the specified {@code name} as its name, and belongs to the
	 * thread group referred to by {@code group}, and has the specified <i>stack
	 * size</i>.
	 *
	 * <p>
	 * This constructor is identical to {@link #Thread(ThreadGroup,Runnable,String)}
	 * with the exception of the fact that it allows the thread stack size to be
	 * specified. The stack size is the approximate number of bytes of address space
	 * that the virtual machine is to allocate for this thread's stack. <b>The
	 * effect of the {@code stackSize} parameter, if any, is highly platform
	 * dependent.</b>
	 *
	 * <p>
	 ...
	 * throwing a {@link StackOverflowError}. Similarly, specifying a lower value
	 * may allow a greater number of threads to exist concurrently without throwing
	 * an {@link OutOfMemoryError} (or other internal error). The details of the
	 * relationship between the value of the <tt>stackSize</tt> parameter and the
	 * maximum recursion depth and concurrency level are platform-dependent. <b>On
	 * some platforms, the value of the {@code stackSize} parameter may have no
	 * effect whatsoever.</b>
	 *
	 * <p>
	 * The virtual machine is free to treat the {@code stackSize} parameter as a
	 ...
	 *
	 * <p>
	 * Specifying a value of zero for the {@code stackSize} parameter will cause
	 * this constructor to behave exactly like the
	 * {@code Thread(ThreadGroup, Runnable, String)} constructor.
	 *
	 * <p>
	 * <i>Due to the platform-dependent nature of the behavior of this constructor,
	 ...
	 * JRE implementation on which an application is to run.</i>
	 *
	 * <p>
	 * Implementation note: Java platform implementers are encouraged to document
	 * their implementation's behavior with respect to the {@code stackSize}
	 * parameter.
	 *
	 *
	 * @param group     the thread group. If {@code null} and there is a security
	 ...
	 *                  security manager or {@code
	 *         SecurityManager.getThreadGroup()} returns {@code null}, the group is
	 *                  set to the current thread's thread group.
	 *
	 * @param target    the object whose {@code run} method is invoked when this
	 *                  thread is started. If {@code null}, this thread's run method
	 *                  is invoked.
	 *
	 * @param name      the name of the new thread
	 *
	...
	public synchronized void start() {
		 ...
		 * threads created/set up by the VM. Any new functionality added to this method
		 * in the future may have to also be added to the VM.
		 *
	 ...
	 * <p>
	 * If there is a security manager installed, its <code>checkAccess</code> method
	 * is called with <code>this</code> as its argument. This may result in a
	 * <code>SecurityException</code> being raised (in the current thread).
	 * <p>
	 * If this thread is different from the current thread (that is, the current
	 * thread is trying to stop a thread other than itself), the security manager's
	 * <code>checkPermission</code> method (with a
	 * <code>RuntimePermission("stopThread")</code> argument) is called in addition.
	 * Again, this may result in throwing a <code>SecurityException</code> (in the
	 * current thread).
	 * <p>
	 * The thread represented by this thread is forced to stop whatever it is doing
	 * abnormally and to throw a newly created <code>ThreadDeath</code> object as an
	 * exception.
	 * <p>
	 ...
	 * <p>
	 * An application should not normally try to catch <code>ThreadDeath</code>
	 * unless it must do some extraordinary cleanup operation (note that the
	 * throwing of <code>ThreadDeath</code> causes <code>finally</code> clauses of
	 * <code>try</code> statements to be executed before the thread officially
	 * dies). If a <code>catch</code> clause catches a <code>ThreadDeath</code>
	 * object, it is important to rethrow the object so that the thread actually
	 * dies.
	 * <p>
	 ...
	 * exception is an instance of <code>ThreadDeath</code>.
	 *
	 ...
	 *             interrupt the wait. For more information, see <a href=
	 ...
	 * @param obj ignored
	 *
	 * @deprecated This method was originally designed to force a thread to stop and
	 *             throw a given {@code Throwable} as an exception. It was
	 *             inherently unsafe (see {@link #stop()} for details), and
	 *             furthermore could be used to generate exceptions that the target
	 *             thread was not prepared to handle. For more information, see
	 *             <a href=
	 *             "{@docRoot}/../technotes/guides/concurrency/threadPrimitiveDeprecation.html">Why
	 *             are Thread.stop, Thread.suspend and Thread.resume
	 *             Deprecated?</a>.
	 ...
	 * status</i> of the thread is cleared by this method. In other words, if this
	 ...
	 *
	 * <p>
	 * A thread interruption ignored because a thread was not alive at the time of
	 * the interrupt will be reflected by this method returning false.
	 *
	...
	/**
	 * Tests whether this thread has been interrupted. The <i>interrupted status</i>
	 ...
	 * not based on the value of ClearInterrupted that is passed.
	 */
	 ...
	 *
	 * @deprecated This method was originally designed to destroy this thread
	 *             without any cleanup. Any monitors it held would have remained
	 *             locked. However, the method was never implemented. If if were to
	 *             be implemented, it would be deadlock-prone in much the manner of
	 *             {@link #suspend}. If the target thread held a lock protecting a
	 *             critical system resource when it was destroyed, no thread could
	 *             ever access this resource again. If another thread ever attempted
	 *             to lock this resource, deadlock would result. Such deadlocks
	 *             typically manifest themselves as "frozen" processes. For more
	 ...
	 * arguments. This may result in throwing a <code>SecurityException </code>(in
	 * the current thread).
	 * <p>
	 ...
	 *             thread can access this resource until the target thread is
	 *             resumed. If the thread that would resume the target thread
	 *             attempts to lock this monitor prior to calling
	 *             <code>resume</code>, deadlock results. Such deadlocks typically
	 *             manifest themselves as "frozen" processes. For more information,
	 *             see <a href=
	 *             "{@docRoot}/../technotes/guides/concurrency/threadPrimitiveDeprecation.html">Why
	 *             are Thread.stop, Thread.suspend and Thread.resume
	 *             Deprecated?</a>.
	 ...
	 * @see #checkAccess
	 * @see #suspend()
	 * @deprecated This method exists solely for use with {@link #suspend}, which
	 ...
	 * {@linkplain java.lang.ThreadGroup thread group} and its subgroups.
	 * Recursively iterates over all subgroups in the current thread's thread group.
	 *
	 * <p>
	 * The value returned is only an estimate because the number of threads may
	 * change dynamically while this method traverses internal data structures, and
	 * might be affected by the presence of certain system threads. This method is
	 * intended primarily for debugging and monitoring purposes.
	 *
	 * @return an estimate of the number of active threads in the current thread's
	 *         thread group and in any other thread group that has the current
	 ...
	 * {@link java.lang.ThreadGroup#enumerate(Thread[])} method of the current
	 * thread's thread group.
	 *
	 * <p>
	 * An application might use the {@linkplain #activeCount activeCount} method to
	 * get an estimate of how big the array should be, however <i>if the array is
	 * too short to hold all the threads, the extra threads are silently
	 * ignored.</i> If it is critical to obtain every active thread in the current
	 * thread's thread group and its subgroups, the invoker should verify that the
	 ...
	 *
	 * @throws SecurityException if {@link java.lang.ThreadGroup#checkAccess}
	 *                           determines that the current thread cannot access
	 *                           its thread group
	 */
	 ...
	 * @deprecated The definition of this call depends on {@link #suspend}, which is
	 *             deprecated. Further, the results of this call were never
	 *             well-defined.
	 */
	@Deprecated
	...
	/**
	 * Waits at most {@code millis} milliseconds for this thread to die. A timeout
	 * of {@code 0} means to wait forever.
	 *
	 * <p>
	 * This implementation uses a loop of {@code this.wait} calls conditioned on
	 * {@code this.isAlive}. As a thread terminates the {@code this.notifyAll}
	 * method is invoked. It is recommended that applications not use {@code wait},
	...
	/**
	 * Waits at most {@code millis} milliseconds plus {@code nanos} nanoseconds for
	 * this thread to die.
	 *
	 * <p>
	 * This implementation uses a loop of {@code this.wait} calls conditioned on
	...
	/**
	 * Marks this thread as either a {@linkplain #isDaemon daemon} thread or a user
	 ...
	 *
	 * @throws IllegalThreadStateException if this thread is {@linkplain #isAlive
	 *                                     alive}
	 *
	 * @throws SecurityException           if {@link #checkAccess} determines that
	 *                                     the current thread cannot modify this
	 *                                     thread
	 */
	 ...
	 * thread.
	 * <p>
	 * If there is a security manager, its <code>checkAccess</code> method is called
	 * with this thread as its argument. This may result in throwing a
	 * <code>SecurityException</code>.
	 *
	 * @exception SecurityException if the current thread is not allowed to access
	 *                              this thread.
	 * @see SecurityManager#checkAccess(Thread)
	 */
	 ...
	 * when loading classes and resources. If not {@linkplain #setContextClassLoader
	 * set}, the default is the ClassLoader context of the parent Thread. The
	 * context ClassLoader of the primordial thread is typically set to the class
	 * loader used to load the application.
	 *
	 * <p>
	 * If a security manager is present, and the invoker's class loader is not
	 * {@code null} and is not the same as or an ancestor of the context class
	 * loader, then this method invokes the security manager's
	 * {@link SecurityManager#checkPermission(java.security.Permission)
	 * checkPermission} method with a {@link RuntimePermission
	 * RuntimePermission}{@code
	 ...
	 *         loader)
	 *
	 * @throws SecurityException if the current thread cannot get the context
	 *                           ClassLoader
	 *
	 * @since 1.2
	 */
	 ...
	 * the appropriate class loader, through {@code getContextClassLoader}, to code
	 * running in the thread when loading classes and resources.
	 *
	 * <p>
	 * If a security manager is present, its
	 * {@link SecurityManager#checkPermission(java.security.Permission)
	 * checkPermission} method is invoked with a {@link RuntimePermission
	 * RuntimePermission}{@code
	 ...
	 *
	 * @throws SecurityException if the current thread cannot set the context
	 *                           ClassLoader
	 *
	 * @since 1.2
	 */
	 ...
	 * element of the array represents the top of the stack, which is the most
	 * recent method invocation in the sequence. The last element of the array
	 * represents the bottom of the stack, which is the least recent method
	 * invocation in the sequence.
	 *
	 * <p>
	 * If there is a security manager, and this thread is not the current thread,
	 * then the security manager's <tt>checkPermission</tt> method is called with a
	 * <tt>RuntimePermission("getStackTrace")</tt> permission to see if it's ok to
	 * get the stack trace.
	 *
	 * <p>
	 * Some virtual machines may, under some circumstances, omit one or more stack
	 * frames from the stack trace. In the extreme case, a virtual machine that has
	 * no stack trace information concerning this thread is permitted to return a
	 * zero-length array from this method.
	 *
	 * @return an array of <tt>StackTraceElement</tt>, each represents one stack
	 *         frame.
	 *
	 ...
	 * and each map value is an array of <tt>StackTraceElement</tt> that represents
	 * the stack dump of the corresponding <tt>Thread</tt>. The returned stack
	 * traces are in the format specified for the {@link #getStackTrace
	 ...
	 * <tt>RuntimePermission("modifyThreadGroup")</tt> permission to see if it is ok
	 * to get the stack trace of all threads.
	 *
	 * @return a <tt>Map</tt> from <tt>Thread</tt> to an array of
	 *         <tt>StackTraceElement</tt> that represents the stack trace of the
	...
	/**
	 * Verifies that this (possibly subclass) instance can be constructed without
	 * violating security constraints: the subclass must not override
	 ...
	 * override security-sensitive non-final methods. Returns true if the subclass
	 * overrides any of the methods, false otherwise.
	 */
	 ...
	 * specified waiting time is in this state.</li>
	 ...
	 * virtual machine states which do not reflect any operating system thread
	 * states.
	 *
	 * @since 1.5
	 * @see #getState
	...
	/**
	 * Interface for handlers invoked when a <tt>Thread</tt> abruptly terminates due
	 ...
	 * <tt>UncaughtExceptionHandler</tt> using {@link #getUncaughtExceptionHandler}
	 * and will invoke the handler's <tt>uncaughtException</tt> method, passing the
	 * thread and the exception as arguments. If a thread has not had its
	 * <tt>UncaughtExceptionHandler</tt> explicitly set, then its
	 * <tt>ThreadGroup</tt> object acts as its <tt>UncaughtExceptionHandler</tt>. If
	 * the <tt>ThreadGroup</tt> object has no special requirements for dealing with
	 * the exception, it can forward the invocation to the
	 * {@linkplain #getDefaultUncaughtExceptionHandler default uncaught exception
	 * handler}.
	 *
	 ...
	 * thread's {@link ThreadGroup} object and finally by the default uncaught
	 ...
	 * does not specialize its <tt>uncaughtException</tt> method, then the default
	 * handler's <tt>uncaughtException</tt> method will be invoked.
	 * <p>
	 * By setting the default uncaught exception handler, an application can change
	 * the way in which uncaught exceptions are handled (such as logging to a
	 * specific device, or file) for those threads that would already accept
	 * whatever &quot;default&quot; behavior the system provided.
	 *
	 * <p>
	 * Note that the default uncaught exception handler should not usually defer to
	 * the thread's <tt>ThreadGroup</tt> object, as that could cause infinite
	 * recursion.
	 *
	 * @param eh the object to use as the default uncaught exception handler. If
	 ...
	 * an uncaught exception. If the returned value is <tt>null</tt>, there is no
	 * default.
	 * 
	 * @since 1.5
	 * @see #setDefaultUncaughtExceptionHandler
	 ...
	 * explicitly set then this thread's <tt>ThreadGroup</tt> object is returned,
	 * unless this thread has terminated, in which case <tt>null</tt> is returned.
	 * 
	 * @since 1.5
	 ...
	 * uncaught exception.
	 * <p>
	 * A thread can take full control of how it responds to uncaught exceptions by
	 * having its uncaught exception handler explicitly set. If no such handler is
	 * set then the thread's <tt>ThreadGroup</tt> object acts as its handler.
	 * 
	 ...
	 * @throws SecurityException if the current thread is not allowed to modify this
	 *                           thread.
	 * @see #setDefaultUncaughtExceptionHandler
	 * @see ThreadGroup#uncaughtException
	 * @since 1.5
	...
	/**
	 * Dispatch an uncaught exception to the handler. This method is intended to be
	...
	/**
	 * Removes from the specified map any keys that have been enqueued on the
	...
	static class WeakClassKey extends WeakReference<Class<?>> {
		/**
		 * saved value of the referent's identity hash code, to maintain a consistent
		 * hash code after the referent has been cleared
		 */
		...
		/**
		 * Returns true if the given object is this identical WeakClassKey instance, or,
		 * if this object's referent has not been cleared, if the given object is
		 * another WeakClassKey instance with the identical non-null referent as this
		 * one.
		 */
		@Override
